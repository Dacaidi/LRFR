...
class Agent(nn.Module):
    ...
    def train_epoch(self, train_loader, epoch, count_cls_step):
        ...
        for i, (inputs, target, task) in enumerate(train_loader):
            ...
            if self.regularization:
                self.update_batch_normalization()
    ...
    def criterion(self, preds, targets, tasks, regularization=True):
        ...
        if regularization and len(self.regularization_terms) > 0:
            # Calculate the reg_loss only when the regularization_terms exists (for EWC)
    ...
    def update_batch_normalization(self):
        # add etra regularization to the batch normalization parameters gradients
        for m in self.model.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.weight.grad.data.add_(self.regularization_weight * torch.sign(m.weight.data))  # L1 gradient
