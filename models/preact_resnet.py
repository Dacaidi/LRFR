...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, droprate=0, cfg=None):
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3, cfg=None):
        ...
        if cfg:
            ...
            self.stage1 = self._make_layer_with_configuration(block, 64, num_blocks[0], stride=1, cfg=cfg[0:4])
            self.stage2 = self._make_layer_with_configuration(block, 128, num_blocks[1], stride=2, cfg=cfg[4:8])
            self.stage3 = self._make_layer_with_configuration(block, 256, num_blocks[2], stride=2, cfg=cfg[8:12])
            self.stage4 = self._make_layer_with_configuration(block, 512, num_blocks[3], stride=2, cfg=cfg[12:16])
    ...
    def _make_layer_with_configuration(self, block, planes, num_blocks, stride, cfg):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = [block(self.in_planes, planes, strides[0], cfg=cfg[0:2])]
        self.in_planes = planes * block.expansion
        layers.append(block(self.in_planes, planes, strides[1], cfg=cfg[2:4]))
        self.in_planes = planes * block.expansion
        return nn.Sequential(*layers)
